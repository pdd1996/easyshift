...
- 小程序是否已绑定不由 `users.status` 单独判断，而由 `role = staff` 且 `wx_openid`、`employee_id` 均有值共同决定。
- `users.status = active` 表示该登录账号可用；`users.status = disabled` 表示账号停用。停用账号仍可能保留绑定关系，但登录与 Token 鉴权必须失败。
- 本地或测试中要重置小程序为“未绑定”，应清理 staff 用户的 `wx_openid` / `employee_id` 绑定关系，并更新 `token_valid_after` 使旧 Token 失效；不要只把 `users.status` 改成 `disabled`。
- `employees.status` 是员工档案状态，取值为 `active` / `inactive`；`inactive` 表示员工停用或离职，不应继续绑定、排班或展示为可排班人员。
...
| 小程序绑定状态 | `users`                                      | `role = staff` 且 `wx_openid`、`employee_id` 均有值；`status` 仅表示账号是否可登录 |
...
| 版本   | 日期         | 说明               |
| ---- | ---------- | ---------------- |
| v1.2 | 2026-06-27 | 补充小程序绑定状态、`users.status` 与 `employees.status` 语义 |
